    ...
    if request.method == 'POST':
        if 'atualizar' in request.form:
            ...
            result = atualizaDadosTurma(
                codTurma=request.form["cod_turma"], dadosTurma=dados)
            if result["success"] == 6:
                flash(result["message"], "success")
            else:
                flash(result["message"], "danger")
        if 'apagar' in request.form:
            result = excluiTurma(codTurma=request.form["codTurma"])
            if result["success"] == 9:
                flash(result["message"], "success")
            else:
                flash(result["message"], "danger")
        elif 'consulta' in request.form:
            result = consultaTurma(codTurma=request.form["codTurma"])
            if result["success"] == 3:
                flash(result["message"], "success")
                turma = result["turma"]
            else:
            ...
            result = addAlunoTurma(matrAluno=request.form["matricula"], codTurma=request.form["turma"])
            if result["success"] == 6:
                flash(result["message"], "success")
            else:
                flash(result["message"], "danger")
        elif 'deletar' in request.form:
            result = removeAlunoTurma(matrAluno=request.form["matricula"], codTurma=request.form["turma"])
            if result["success"] == 9:
                flash(result["message"], "success")
            else:
                flash(result["message"], "danger")
